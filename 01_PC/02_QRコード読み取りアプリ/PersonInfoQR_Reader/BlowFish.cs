...
namespace EncryptedQRCodeReader
{
    class BlowFish
    {
        ...
        public byte[] Encrypt_CBC_bytes(string pt)
        {
            if (!IVSet)
                SetRandomIV();

            // byte[] の結合処理をString -> byte[]の方で代用
            String strEncrypt = ByteToHex(InitVector) + ByteToHex(Encrypt_CBC(Encoding.UTF8.GetBytes(pt)));
            return HexToByte(strEncrypt);
        }
        ...
        public byte[] Decrypt_CBC_bytes(string ct)
        {
            // byte[] の結合処理をString -> byte[]の方で代用
            String strDecrypt = ByteToHex(InitVector) + ByteToHex(Decrypt_CBC(HexToByte(ct.Substring(16))));
            return HexToByte(strDecrypt);
        ...
        // kojima add function
        public byte[] Encrypt_ECB_bytes(string pt)
        {
            return Encrypt_ECB(Encoding.UTF8.GetBytes(pt));
        }

        /// <summary>
        /// Encrypt a string in ECB mode(SJIS用)
        /// </summary>
        /// <param name="pt">Plaintext to encrypt as ascii string</param>
        /// <returns>hex value of encrypted data</returns>
        public string Encrypt_ECB_SJIS(string pt)
        {
            return ByteToHex(Encrypt_ECB(Encoding.GetEncoding("Shift_JIS").GetBytes(pt)));
        }
        // kojima add function
        public byte[] Encrypt_ECB_bytes_SJIS(string pt)
        {
            return Encrypt_ECB(Encoding.GetEncoding("Shift_JIS").GetBytes(pt));
        }
        ...
        public string Decrypt_ECB(string ct)
        {
            ...
            return Encoding.UTF8.GetString(Decrypt_ECB(HexToByte(ct))).TrimEnd('\0');
        }

        /// <summary>
        /// Decrypts a string (ECB)(SJIS用)
        /// </summary>
        /// <param name="ct">hHex string of the ciphertext</param>
        /// <returns>Plaintext ascii string</returns>
        public string Decrypt_ECB_SJIS(string ct)
        {
            //return Encoding.GetEncoding("Shift_JIS").GetString(Decrypt_ECB(HexToByte(ct))).Replace("\0", "");
            return Encoding.GetEncoding("Shift_JIS").GetString(Decrypt_ECB(HexToByte(ct))).TrimEnd('\0');
        }
